...
import com.igeltech.nevercrypt.crypto.AF;
...
import com.igeltech.nevercrypt.crypto.engines.GOSTCBC;
import com.igeltech.nevercrypt.crypto.engines.GOSTXTS;
import com.igeltech.nevercrypt.crypto.engines.SerpentCBC;
import com.igeltech.nevercrypt.crypto.engines.SerpentXTS;
import com.igeltech.nevercrypt.crypto.engines.TwofishCBC;
...
import com.igeltech.nevercrypt.exceptions.UnsupportedContainerTypeException;
...
public class VolumeLayout extends VolumeLayoutBase
{
    ...
    public void writeKey(RandomAccessIO output, int keyIndex, byte[] password) throws IOException, ApplicationException
    {
        checkWriteHeaderPrereqs();
    ...
    protected void writeKey(RandomAccessIO output, KeySlot ks, byte[] password) throws IOException, ApplicationException
    {
        try
        {
    ...
    protected byte[] getCipherName()
    {
        ...
        System.arraycopy(cn.getBytes(), 0, res, 0, Math.min(cn.length(), MAX_CIPHERNAME_LEN));
        return res;
    }
    ...
    protected byte[] getCipherModeName()
    {
        ...
        System.arraycopy(cn.getBytes(), 0, res, 0, Math.min(cn.length(), MAX_CIPHERMODENAME_LEN));
        return res;
    }
    ...
    protected byte[] getUUIDBytes()
    {
        ...
        System.arraycopy(uuidStr.getBytes(), 0, res, 0, Math.min(uuidStr.length(), UUID_LENGTH));
        return res;
    }
    ...
    protected byte[] getHashspecName()
    {
        ...
        System.arraycopy(cn.getBytes(), 0, res, 0, Math.min(cn.length(), MAX_HASHSPEC_LEN));
        return res;
    }
    ...
    private static final byte[] MAGIC = new byte[]{'L', 'U', 'K', 'S', (byte) 0xba, (byte) 0xbe};
}
