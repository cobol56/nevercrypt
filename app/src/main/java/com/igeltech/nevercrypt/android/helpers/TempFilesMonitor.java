...
public class TempFilesMonitor
{
    ...
    {
        Location loc = monitored ? FileOpsService.getMonitoredMirrorLocation(workDir, context, rootLocation.getId()) : FileOpsService.getNonMonitoredMirrorLocation(workDir, context, rootLocation.getId());
        loc.setCurrentPath(PathUtil.getDirectory(loc.getCurrentPath(), SimpleCrypto.calcStringMD5(path.getPathString())).getPath());
        return loc;
    }
            ...
            while (!_stop)
            {
                synchronized (_syncObject)
                {
                    ...
                    while (iter.hasNext())
                    {
                        ...
                        try
                        {
                            if (!fileInfo.devicePath.getCurrentPath().exists() || (fileInfo.srcFolderLocation instanceof Openable && !((Openable) fileInfo.srcFolderLocation).isOpen()))
                                iter.remove();
