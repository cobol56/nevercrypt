...
public abstract class UserSettingsCommon implements SettingsCommon
{
    ...
    {
        return _prefs.getString(LOCATION_SETTINGS_PREFIX + locationId, _defaultSettings.getLocationSettingsString(locationId));
    }
    ...
    {
        return _prefs.getString(LOCATIONS_LIST, _defaultSettings.getStoredLocations());
    }
    ...
    {
        return _prefs.getInt(MAX_FILE_SIZE_TO_OPEN, _defaultSettings.getMaxTempFileSize());
    }
    ...
    {
        _prefs.edit().putString(key, SimpleCrypto.encrypt(getSettingsProtectionKey(), value.getBytes())).commit();
    }

    public void setProtectedField(String key, byte[] value) throws InvalidSettingsPassword
    {
        _prefs.edit().putString(key, SimpleCrypto.encrypt(getSettingsProtectionKey(), value)).commit();
    }
    ...
    public synchronized void saveSettingsProtectionKey() throws InvalidSettingsPassword
    {
        ...
        try
        {
            ...
            if (pass != null)
            {
                try
                {
                    _prefs.edit().putString(SETTINGS_PROTECTION_KEY_USER, SimpleCrypto.encryptWithPassword(pass, key)).
            ...
            else
            {
                ...
                try
                {
                    _prefs.edit().putString(SETTINGS_PROTECTION_KEY_AUTO, SimpleCrypto.encryptWithPassword(pass, key)).
